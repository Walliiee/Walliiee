<!-- Banner -->
![Walliiee](https://github.com/user-attachments/assets/5173e146-1539-4ff9-aa8b-d746b3d2b5b0)

<div align="center">

<!-- Animated typing header -->
[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=24&pause=1200&color=58A6FF&center=true&vCenter=true&width=800&height=55&lines=Hulk%2C+Thor%2C+Wall-E...+or+just+Walliiee+%F0%9F%91%8B%3BGreat+Listener+%C2%B7+Always+Curious%3BChallenge+Seeker+%C2%B7+Master+of+one%2C+Jack+of+many%3BBuilding+stuff+%26+learning+every+darn+day)](https://git.io/typing-svg)

**🧠 GenAI / RAG builder · 🏗️ shipping fitness + fintech side projects · ☕ coffee & sourdough at 100%, everything else at 80%**

<br/>

<!-- Contact / socials -->
[![Email](https://img.shields.io/badge/Email-info@mikefit.dk-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:info@mikefit.dk)
[![Website](https://img.shields.io/badge/Website-mikekrohn.ai-1f6feb?style=flat-square&logo=googlechrome&logoColor=white)](https://mikekrohn.ai/)
[![Challenger Crew](https://img.shields.io/badge/🚀_App-Challenger_Crew-7c3aed?style=flat-square)](https://www.challenger-crew.com/)

<br/>

<!-- Flavor / "powered by" badges -->
![Fueled by](https://img.shields.io/badge/Fueled_by-%E2%98%95_+_%F0%9F%8D%9E_sourdough-6F4E37?style=flat-square)
![80/20 certified](https://img.shields.io/badge/80%2F20-certified-2EA44F?style=flat-square)
![Dad mode](https://img.shields.io/badge/Dad_mode-ON-3FB950?style=flat-square)
![All in](https://img.shields.io/badge/When_I_start-I_go_all_in-58A6FF?style=flat-square)
<!-- TODO: add your LinkedIn handle below, then uncomment -->
<!-- [![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/your-handle) -->

</div>

---

> *If you're reading this, it's too late.*

Prior to December 2024, I hadn't built anything — only a foundational knowledge of software development and design.
Now I build stuff and I learn every darn day, and it's exactly how I want it to be.

> “Much to learn, he still has.”
> – *Yoda*

## ✨ About Me

🎂 **Age:** <!--START_AGE-->33 years, 1 months, 7 days, 486 minutes<!--END_AGE--> &nbsp;<sub>↑ auto-updated daily by a GitHub Action, because why round? 🤖</sub>

- 👨‍👧 Father, good friend, and carrier of *a hint of too much empathy* — which makes me want to solve all of the world's problems and help everybody I meet.
- 🧭 I like to **learn**, I like to **explore**, and I like a good **challenge**.
- 🔁 When I start something I go all in — I listen, read and do until I understand, then I keep iterating.

> **The 80/20 Principle:** Most things in life you don't need to master beyond 80%, and you can get there with 20% of the effort. Pick **one, max two** things to fully focus on; the rest stay at 80/20. For me the 100% goes to great coffee and sourdough — the rest is for the margins and the specialists.

> “Master of one, Jack of many.”
> – *ME*

## 🔧 Languages & Tools

<sub>not the best, not the worst — just shipping</sub>

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat-square&logo=css3&logoColor=white)
![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white) <sub>…but really, no thank you</sub>

**Frontend**
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![Bootstrap](https://img.shields.io/badge/Bootstrap-7952B3?style=flat-square&logo=bootstrap&logoColor=white)

**Backend**
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)

**Tools & Environment**
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)
![Cursor](https://img.shields.io/badge/Cursor-000000?style=flat-square&logo=cursor&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)

**Also into:** staying healthy in mind and body 🏃 · football & American football 🏈 · and GenAI — because *damn*, can we make some awesome things happen.

## 🚀 What I'm Building

| Project | What it is | Status |
| --- | --- | --- |
| **Markets Models** | 6 RAG models, each with tailored system prompts & parameters | ✅ Built |
| **Heatmap Project** | Visualizing GenAI conversation usage (Claude + ChatGPT) on a calendar heatmap with Plotly Calplot — plus embedding & clustering to classify conversation topics on upload | 🔒 Internal |
| **[Challenger Crew](https://www.challenger-crew.com/)** | Fitness community app for training & accountability | 🟢 Live |
| **Small Cap Analyzer Blog** | Analysis + writing on small-cap markets | 🚧 In progress |

- 🌱 **Currently learning:** how to build agents
- ⚡ **Fun fact:** before going into IT I took a degree in the US — and came back to Denmark with a serious outdoor habit and a love for the game.

## 🗺️ My Journey

```mermaid
timeline
  title From the gridiron to GenAI
  section 🇺🇸 United States
    2012 : Left for California to play football
    2014 : AA in Kinesiology
    2015 : Moved to the East Coast
    2017 : Summer internship → Graduate Assistant
  section 🇩🇰 Back to Denmark
    2018 : Moved home
    2020 : Fell in love ❤️
  section 💻 Into Tech
    2022 : Economics and IT · first taste of Python
    2024 : Decided to actually start building
    2025 : Internship @ KPMG NewTech · shipped my first web app
```

## 🃏 Dad Joke of the Day

<div align="center">

<em><!--START_DADJOKE-->A woman is on trial for beating her husband to death with his guitar collection. Judge says, ‘First offender?’ She says, ‘No, first a Gibson! Then a Fender!’<!--END_DADJOKE--></em>

<sub>↑ a fresh one served daily by a GitHub Action 🤖 — sorry in advance</sub>

</div>

## 📊 GitHub in Numbers

<div align="center">

![Walliiee's GitHub stats](https://github-readme-stats.vercel.app/api?username=Walliiee&show_icons=true&theme=tokyonight&hide_border=true&count_private=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Walliiee&layout=compact&theme=tokyonight&hide_border=true&langs_count=8)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=Walliiee&theme=tokyonight&hide_border=true)

<!-- Contribution activity over time -->
[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Walliiee&theme=tokyo-night&hide_border=true&area=true&custom_title=Contribution%20Graph)](https://github.com/Ashutosh00710/github-readme-activity-graph)

<!-- Achievement trophies -->
[![Trophies](https://github-profile-trophy.vercel.app/?username=Walliiee&theme=tokyonight&no-frame=true&no-bg=true&column=4&margin-w=4&margin-h=4)](https://github.com/ryo-ma/github-profile-trophy)

<!-- Contribution snake — activates after the "Generate Snake" Action runs once -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Walliiee/Walliiee/output/github-snake-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Walliiee/Walliiee/output/github-snake.svg">
  <img alt="Contribution snake animation" src="https://raw.githubusercontent.com/Walliiee/Walliiee/output/github-snake.svg">
</picture>

</div>

---

<div align="center">

*I'm here, so are you! Why not make the most of it? Help me grow.* 🌱

**You're visitor number**

![Visitor Count](https://profile-counter.glitch.me/Walliiee/count.svg)

</div>
